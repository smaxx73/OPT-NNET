...
def descent(f, grad_f, X0, delta=0.1, nmax=10):
    liste_X = [X0]
    liste_grad = []
    X = X0
    for i in range(nmax):
...
def display_descent(f, grad_f, X0, delta=0.1, nmax=10):
    liste_X, liste_grad = descent(f, grad_f, X0, delta=delta, nmax=nmax)
    print("step value",delta)
    print("Number of iterations", nmax)
    print("initial point", X0)
    for i in range(len(liste_X)-1):    # flèches
        print("--- stage",i)
        print("Point :", *liste_X[i])
        print("Gradient ", *liste_grad[i])
        print("Value of function ", f(*liste_X[i]))
    print("last point :", *liste_X[-1])
    print("Last value of function ", f(*liste_X[-1]))
    return


def graphic_descent_2var_2d(f, grad_f, X0, delta=0.1, nmax=10, zone = (-3.0,3.0,-3.0,3.0)):
...
def graphic_regression(f, grad_f, X0, points, delta=0.1, nmax=10, zone = (-3.0,3.0,-3.0,3.0)):
    # 1. Droites
    liste_X, liste_grad = descent(f, grad_f, X0, delta=delta, nmax=nmax)
...
def graphic_descent_1var(f, grad_f, X0, delta=0.1, nmax=10):
    xmin, xmax = -1.5, 2.5
